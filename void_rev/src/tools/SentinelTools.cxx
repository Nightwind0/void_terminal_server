...
void SentinelTools::DeploySentinelsAndUnload(int num, PlayerHandle * pPlayer, ShipHandle * pFrom, int sector)
{
    ...
    std::string sql = "select sum(ncount) from sentinels where ksector = '" + IntToString ( sector )  + "' and kplayer = '" + PrepareForSQL(player) + "\';";
...
void SentinelTools::RemoveSentinels(int num, const std::string &player, int sector)
{
    ...
    {
	std::string owner = PQgetvalue(dbresult,i,0);
	int sentinels = atoi(PQgetvalue(dbresult,i,1));
	
	if(sentinels > num)
	{
	    // Subtract from this one
	    int newcount = sentinels - num;
	    std::string update = "update sentinels set ncount = " + IntToString(newcount) + " where kplayer = '" + owner + "' and ksector = '" 
		+ IntToString(sector) + "';";
	...
	{
	    // Use this one up entirely and nuke it
	    std::string delete_stmt = "delete from sentinels where ksector = '" + IntToString(sector) +
		"' and kplayer = '" + owner + "';";

	    PGresult * updateresult = DBExec(delete_stmt);
