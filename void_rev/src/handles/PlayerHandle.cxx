...
const char * PlayerHandle::FIELD_NAMES[] = {"sname","klogin","kmob","ncredits","nturnsleft","npoints","kalliance","kcurrentship", "dfirstplay", "dlastplay", "bdead"};
...
void PlayerHandle::LoadFromDB()
{
    ...
    std::string query = "select sname,klogin,kmob,ncredits,nturnsleft,npoints,kalliance,kcurrentship,dfirstplay,dlastplay from player where " + CreateWhereClause() + ";";
    ...
    m_fields[MOB] = std::make_shared<Text>(FIELD_NAMES[MOB],r[0][2].as<std::string>(), r[0][2].is_null());
    m_fields[CREDITS] = std::make_shared<Integer>(FIELD_NAMES[CREDITS],r[0][3].as<std::string>(), r[0][3].is_null());
    m_fields[TURNSLEFT] = std::make_shared<Integer>(FIELD_NAMES[TURNSLEFT],r[0][4].as<std::string>(), r[0][4].is_null());
    m_fields[POINTS] = std::make_shared<Integer>(FIELD_NAMES[POINTS],r[0][5].as<std::string>(), r[0][5].is_null());
    m_fields[ALLIANCE] = std::make_shared<Text>(FIELD_NAMES[ALLIANCE],r[0][6].as<std::string>(), r[0][6].is_null());
    m_fields[CURRENTSHIP] = std::make_shared<Integer>(FIELD_NAMES[CURRENTSHIP],r[0][7].as<std::string>(), r[0][7].is_null());
    m_fields[FIRSTPLAY] = std::make_shared<Timestamp>(FIELD_NAMES[FIRSTPLAY],r[0][8].as<std::string>(), r[0][8].is_null());
    m_fields[LASTPLAY] = std::make_shared<Timestamp>(FIELD_NAMES[LASTPLAY],r[0][9].as<std::string>(), r[0][9].is_null());
    work.commit();
}
