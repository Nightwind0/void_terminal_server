...
VoidCommandAttack::VoidCommandAttack(VoidServerThread *thread):VoidCommand(thread)
{
}
...
{
    std::string deletestmt = "update player set kcurrentship = null where sname = (select sname from player where kcurrentship = '" + IntToString(ship) + "');";
...
int VoidCommandAttack::CreateEscapePodForPlayer(const std::string player)
{

    std::string nextvalsql = "select nextval('ship_nkey_seq');";
    
    PGresult *dbresult = get_thread()->DBExec(nextvalsql);
    ...
    // TODO: Get escape pode type from config table
     ...
     dbresult = get_thread()->DBExec(stmt);
    ...
    dbresult = get_thread()->DBExec(sit);
