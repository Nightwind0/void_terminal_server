...
def connect_sectors(numsectors):
        ...
        cur = conn.cursor()
        random_points = []
        radius = 20.0 * math.sqrt(numsectors);
        for n in range(0,numsectors):
        ...
        # then, when connecting sectors, we randomly pick a number of neighboring sectors to connect to (normal random centered about C/2) where C is the number of average connections you want a sector to have.
        for sector in range(0,numsectors):
                conn_count = random.gauss(connectivity / 2, 2)
                adj_num = 0
                for i in range(0, conn_count):
                        adjs = set([])
                        spread = calc_spread(10, 10)
                        adj = math.gauss(sector, spread)
                        if adj != sector and adj >= 0 and adj < numsectors and !adj in adjs:
